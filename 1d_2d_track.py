...
"""Ion track and 1D/2D ion occupancy analysis for ion-channel MD simulations.

This script uses MDAnalysis to:
  - Track ion z-positions (pore axis) over time (Ion Track)
  - Compute 1D axial ion occupancy
  - Compute 2D radial–axial ion density/occupancy (with radial-shell normalization)
  - Generate publication-ready PDF figures

User-editable settings include:
  - Trajectory/reference file paths
  - Ion selection (e.g., K+, Na+, Cl-)
  - Alignment atom selection for the channel
  - Cylindrical analysis region (height/depth/radius)
  - Time step parameters (dt, outputstep, skip)
  - Optional snapshot overlay parameters (image path, pixel scaling)

Originally developed for potassium channel simulations; adaptable to other systems.
...
# MAIN ANALYSIS BLOCK
#
# All system-dependent parameters are grouped below.
# Adapt the following sections to your simulation system:
#   - Trajectory path(s)
#   - Ion selection
#   - Alignment atom selection
#   - Cylinder dimensions
#   - Time step / output stride settings
#
# The analysis can be extended to other species (e.g., anions, water)
# by defining additional MDAnalysis selections and output arrays.
##############################################################################
...
# Define a cylindrical analysis region centered at the channel COM.
# Only ions inside this cylinder will be tracked and contribute to occupancy.
#
# height: upper boundary along pore axis (Å)
# depth:  lower boundary along pore axis (Å)
# radius: radial cutoff from pore axis (Å)
height=60 # from 50 Å above
depth=-60 # till 30 Å below the COM
radius=40 # with a radius of 15 Å

### get the number of ions in the system (Normally it is "K" or "POT" for potassium of "NA" or "SOD" for sodium ("CL" or "CLA" for Cl-))
...
# NOTE:
# The analysis can be duplicated for other species (e.g., Cl-, water)
# by creating additional selections and arrays analogous to the ion setup above.
...
for k, ts in enumerate(traj.trajectory):
    ...
    if(k%skip==0):
        ...
        for u in [0]:
            ...
            # copy the current position of ion within the defined cylindric space around the channels COM
            ...
            for i in np.arange(ion[u].shape[0]): 
                ...
                # check further if this ion is really within the analysis borders
                if (z>0 and z<(height-depth)): 
                    # Map MDAnalysis/PDB atom indexing to a continuous ion ID (0 → nIons-1).
# This is necessary because PDB numbering starts at 1 and ions may be grouped
# by compartment or written in separate blocks.
                    ...
                    if u==0: 
                        ...
                        # subtract one because pdb starts with atom id 1 and MDAnalysis starts with atom id 0
                        ni=ions_u[i].ix
                    else:
                        ni=ions_l[i].ix   
                    # depending on the order of your atoms in the pdb you might need to subtract just one number (if you ions are subsequently written in the pdb) or you need to distinguish between the ions you inserted in the two comparments (so they are separate in the pdb and have very different numbers. thats here the case)
...
##############################################################################
# 2D ION OCCUPANCY NORMALIZATION
#
# Radial bins represent cylindrical shells. Outer shells cover larger volumes
# than inner shells, so raw counts are not directly comparable across radius.
#
# We normalize each radial bin by its shell volume:
#   V_shell = π * height * (r_outer^2 - r_inner^2)
#
# The normalized 2D array approximates a density-like quantity (ions per Å^3),
...
### you have to know how many pixel are plotted per pixel in your snapshot
pixPerA=110
...
# A Gaussian filter is applied for plotting aesthetics only and does not
# modify the underlying occupancy arrays used for analysis.
    data2=sp.ndimage.gaussian_filter(data,1)
    # furthermore matplotlib adjusts the scale according the minimum and maximum values of the current data. If you want to compare data inbetween plots you have to ensure, that matplotlib uses the same scale!
